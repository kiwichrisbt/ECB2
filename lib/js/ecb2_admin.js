...
$(function() {
    ...
    // region REPEATER
    // all functions related to the ECB2 Repeater fields
        ...
        stop: function(e,ui) {
            $(this).find('textarea.wysiwyg').each(function() {
                if (typeof tinyMCE!=='undefined') {
                    ...
                    ECB2TinyMCERemove ( $(this).attr('id') );
                    
                    // re-add editor
                    // tinyMCE.execCommand( 'mceAddEditor', false, $(this).attr('id') );
                    ECB2TinyMCEAdd( $(this).attr('id') );
    ...
    // set global variable for TinyMCE config, based on existing TinyMCE settings
    var ecb2TinyMCEConfig = null;
    var isTinyMCE8Plus = null; 
       
    // function to get TinyMCE config from existing active editor - some settings may be missing, can be added as needed
    function ECB2GetTinyMCEConfig () {
        if (ecb2TinyMCEConfig!==null && typeof ecb2TinyMCEConfig==='object' && Object.keys(ecb2TinyMCEConfig).length > 0) {
            return;
        }
        var config_params = [
            'license_key',
        ...
        for (var i=0; i<config_params.length; i++) {
            var param = config_params[i];       
            config[param] = tinymce.activeEditor.options.get(param);
        }
        ecb2TinyMCEConfig = config;
        return;
    };

    // function to check if TinyMCE version is 8 or higher, cache result
    function ECB2isTinyMCE8Plus () {
        if (isTinyMCE8Plus===null) {
            isTinyMCE8Plus = tinymce.majorVersion >= 8 || (tinymce.majorVersion === undefined && parseInt(tinymce.version) >= 8);
        }
        return isTinyMCE8Plus;
    };

    // function to add TinyMCE editor to a textarea, using TinyMCE older <v5 or newer API (v5+)
    function ECB2TinyMCEAdd ( editorId ) {
        if ( ECB2isTinyMCE8Plus() ) {  // TinyMCE 8+ - initialize new editor
            
            ECB2GetTinyMCEConfig();
            tinymce.init({
                selector: '#' + editorId,
                ...ecb2TinyMCEConfig,
                setup: function(editor) {
                    // Call the original setup function to register custom buttons
                    if (ecb2TinyMCEConfig.setup && typeof ecb2TinyMCEConfig.setup === 'function') {
                        ecb2TinyMCEConfig.setup.call(this, editor);
                    }
                    editor.on('init', function() {
                        // make WYSIWYG visible after editor is initialized
            ...
            // TinyMCE 7 and earlier - use execCommand
    ...
    // function to remove TinyMCE editor from a textarea, using TinyMCE older <v5 or newer API (v5+)
    function ECB2TinyMCERemove ( editorId ) {
        if ( ECB2isTinyMCE8Plus() ) {
        ...
        } else {    // TinyMCE 7 and earlier - use execCommand
            // tinyMCE.execCommand( 'mceRemoveEditor', false, $(this).attr('id') );
